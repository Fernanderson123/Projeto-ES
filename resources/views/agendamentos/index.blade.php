...
<head>
    ...
    <style>
        ...
        /* Botão Novo */
        .btn-new {
            background-color: #a78bfa; color: white; border-radius: 50px; padding: 8px 25px;
            text-decoration: none; font-size: 0.9rem; transition: background 0.2s;
            font-family: sans-serif; font-weight: bold; border: 1px solid #a78bfa;
        }
        ...
        /* Status Badges */
        .badge-status { padding: 5px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 600; }
        .status-agendado { background-color: #e0f2fe; color: #0369a1; }
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse ($agendamentos as $agenda)
                                <tr>
                                    <!-- Data Formatada -->
                                    <td class="fw-bold">{{ $agenda->data_hora->format('d/m/Y') }} <span class="text-muted fw-normal">{{ $agenda->data_hora->format('H:i') }}</span></td>
                                    ...
                                    <td class="text-end">
                                        <!-- O botão de editar/excluir virá no próximo passo -->
                                        <button class="btn btn-link p-0 border-0" title="Ver detalhes">
                                            <i class="bi bi-eye action-icon"></i>
                                        </button>
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="6" class="text-center text-muted py-5">Nenhum agendamento encontrado.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
