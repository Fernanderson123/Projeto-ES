...
<head>
    <meta charset="UTF-8">
    <title>Prontuários - Animal Health Center</title>
...
<body>
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="page-title">Prontuário Médico</h2>
        </div>
                    ...
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr>
                                ...
                                <th style="width: 30%;">Resumo Diagnóstico</th>
                        ...
                        <tbody>
                            @forelse ($prontuarios as $prontuario)
                                <tr>
                                    <td class="fw-bold">{{ $prontuario->data_atendimento->format('d/m/Y') }}</td>
                                    <td>
                                        <div>{{ $prontuario->pet->nome }}</div>
                                        <div class="small text-muted">{{ $prontuario->pet->especie }}</div>
                                    </td>
                                    <td>Dr(a). {{ $prontuario->veterinario->name ?? '--' }}</td>
                                    <td class="text-muted text-truncate" style="max-width: 200px;">
                                        {{ Str::limit($prontuario->diagnostico ?? $prontuario->sintomas, 50) }}
                                    </td>
                                    <td class="text-end">
                                        <a href="{{ route('prontuario.edit', $prontuario->id) }}" title="Ver/Editar">
                                            <i class="bi bi-pencil-square action-icon"></i>
                                        </a>
                                        <form action="{{ route('prontuario.destroy', $prontuario->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apagar este registro médico?');">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="5" class="text-center text-muted py-5">Nenhum registro encontrado.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
